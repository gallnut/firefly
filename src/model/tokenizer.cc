...
bool Tokenizer::load(const std::string& path)
{
    ...
    try
    {
        ...
        const auto& pretokenizers = j["pre_tokenizer"]["pretokenizers"];
        for (const auto& pretokenizer : pretokenizers)
        {
            if (pretokenizer.value("type", "") != "Split") continue;
            const auto& pattern = pretokenizer["pattern"];
            if (!pattern.contains("Regex")) continue;
