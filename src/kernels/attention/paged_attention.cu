...
void launch_paged_attention(Tensor& q, Tensor& k, Tensor& v, Tensor& output, void* kv_cache_block_table,
                            ...
                            int max_decode_context_len, cudaStream_t stream)
{
    ...
    if (seq_len > 1 && head_dim == 128)
    {
        ...
        dim3 block(128);
        paged_prefill_wmma_qk_kernel<scalar_t><<<grid, block, 0, stream>>>(
            static_cast<const scalar_t*>(q.data()), static_cast<const scalar_t*>(k.data()),
            static_cast<const scalar_t*>(v.data()), static_cast<const int*>(kv_cache_block_table),
    ...
    paged_attention_kernel<scalar_t><<<grid_decode, block_decode, threads * sizeof(float), stream>>>(
        static_cast<const scalar_t*>(q.data()), static_cast<const scalar_t*>(k.data()),
        static_cast<const scalar_t*>(v.data()), static_cast<const int*>(kv_cache_block_table),
...
void attention(Tensor& q, Tensor& k, Tensor& v, Tensor& output, void* kv_cache_block_table, int kv_head_num,
               ...
               int max_decode_context_len, const device::Context& context)
{
    attention_ex(q, k, v, output, kv_cache_block_table, kv_head_num, seq_len, max_context_blocks, context_lens,
                 AttentionBackend::Auto, prefer_split_decode, max_decode_context_len, -1, context);
}
                  ...
                  const device::Context& context)
{
    ...
    {
        if (kv_cache_block_table != nullptr && seq_len == 1 &&
            attention_backend::launch_flashinfer_decode(q, k, v, output, kv_head_num, head_dim, scale, context))
        {
            cudaError_t err = cudaGetLastError();
            if (err != cudaSuccess)
            {
                throw std::runtime_error(std::string("FlashInfer decode launch failed: ") + cudaGetErrorString(err));
            }
            return;
        }
        if (kv_cache_block_table != nullptr && seq_len > 1 && prefill_context_len >= 0 &&
            attention_backend::launch_flashinfer_prefill(q, k, v, output,
                                                         static_cast<const int*>(kv_cache_block_table), kv_head_num,
                                                         seq_len, max_context_blocks, prefill_context_len, scale,
                                                         context))
        {
            cudaError_t err = cudaGetLastError();
            if (err != cudaSuccess)
            {
                throw std::runtime_error(std::string("FlashInfer prefill launch failed: ") + cudaGetErrorString(err));
            }
            return;
        }
        ...
        if (!warned)
        {
            std::cerr << "FlashInfer does not support this attention shape; falling back to paged attention"
                      << std::endl;
            warned = true;
        }
        backend = kv_cache_block_table == nullptr ? AttentionBackend::Contiguous : AttentionBackend::Paged;
    ...
    else if (backend == AttentionBackend::Paged)
    {
        ...
        else
        {
            launch_paged_attention<half>(q, k, v, output, kv_cache_block_table, kv_head_num, max_context_blocks,
                                         ...
                                         prefer_split_decode, max_decode_context_len, context.stream());
        }
    }
    else
    {
