                    ...
                    const DecodeConfig& decode_config, float scale, const device::Context& context)
{
    ...
    if (use_split_decode)
    {
        return launch_paged_decode(query, key, value, output, options, decode_config, scale, context);
    }
    else
    {
        return launch_paged_prefill(query, key, value, output, options, scale, context);
    }
}
}  // namespace firefly::kernels::attention_detail
