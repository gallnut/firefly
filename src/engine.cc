...
{
    auto req = std::make_shared<GenerationRequest>(id, input_ids, max_tokens, std::move(cancel_flag));
...
void Engine::start()
{
    ...
    // Run dummy forward
    model_->forward(dummy_input, dummy_context_lens, dummy_k, dummy_v, dummy_block_table, dummy_num_blocks, false);
...
void Engine::loop()
{
    ...
    auto ensure_decode_fallback_storage = [this](int current_batch_size, int block_table_elems)
    {
        if (decode_fb_input_capacity_ < current_batch_size)
        {
            decode_fb_input_storage_ = Tensor({current_batch_size, 1}, DType::I32, Device::CUDA);
            decode_fb_input_capacity_ = current_batch_size;
        }
        if (decode_fb_context_capacity_ < current_batch_size)
        {
            decode_fb_context_storage_ = Tensor({current_batch_size}, DType::I32, Device::CUDA);
            decode_fb_context_capacity_ = current_batch_size;
        }
        if (decode_fb_next_token_capacity_ < current_batch_size)
        {
            decode_fb_next_token_storage_ = Tensor({current_batch_size}, DType::I32, Device::CUDA);
            decode_fb_next_token_capacity_ = current_batch_size;
        }
        if (decode_fb_block_table_capacity_ < block_table_elems)
        {
            decode_fb_block_table_storage_ = Tensor({block_table_elems}, DType::I32, Device::CUDA);
    ...
    while (running_)
    {
        ...
        for (const auto& req : batch.failed_requests)
        {
            if (result_queue_)
            {
        ...
        if (!prefill_requests.empty())
        {
            ...
            std::map<int, std::vector<std::shared_ptr<GenerationRequest>>, std::greater<int>> prefill_groups;
            ...
            for (auto& [seq_len, group] : prefill_groups)
            {
                ...
                Tensor d_input({(long)group_size, seq_len}, DType::I32, Device::CUDA);
                cudaMemcpy(d_input.data(), h_input_ids.data(), h_input_ids.size() * sizeof(int), cudaMemcpyHostToDevice);

                Tensor d_context_lens({(long)group_size}, DType::I32, Device::CUDA);
                cudaMemcpy(d_context_lens.data(), h_context_lens.data(), h_context_lens.size() * sizeof(int),
                           cudaMemcpyHostToDevice);
                ...
                cudaMemcpy(d_block_table, h_block_table.data(), h_block_table.size() * sizeof(int),
                           cudaMemcpyHostToDevice);

                if (profile_prefill_enabled())
                {
                    auto& events = prefill_profile_events();
                    cudaEventRecord(events.start, firefly::kernels::get_default_stream());
                }

                Tensor logits =
                    model_->forward(d_input, d_context_lens, k_caches_, v_caches_, d_block_table, max_blocks,
                                    any_prompt_finished, false, 0, min_context_len);

                if (profile_prefill_enabled())
                {
                    auto& events = prefill_profile_events();
                    cudaEventRecord(events.stop, firefly::kernels::get_default_stream());
                ...
                {
                    Tensor next_tokens({(long)group_size}, DType::I32, Device::CUDA);
                    kernels::argmax(logits, next_tokens);
                    std::vector<int> h_next_tokens(group_size);
                    cudaMemcpy(h_next_tokens.data(), next_tokens.data(), h_next_tokens.size() * sizeof(int),
        ...
        if (!decode_requests.empty())
        {
            ...
            auto run_decode_fallback = [&](bool prefer_split_decode)
            {
                ...
                cudaStream_t          profile_stream = firefly::kernels::get_default_stream();
                ...
                cudaMemcpy(d_input.data(), scratch.input_ids.data(), scratch.input_ids.size() * sizeof(int),
                           cudaMemcpyHostToDevice);
                cudaMemcpy(d_context_lens.data(), scratch.context_lens.data(), scratch.context_lens.size() * sizeof(int),
                           cudaMemcpyHostToDevice);
                cudaMemcpy(d_block_table_tensor.data(), scratch.block_table.data(),
                ...
                Tensor logits = model_->forward(d_input, d_context_lens, k_caches_, v_caches_,
                                                static_cast<int*>(d_block_table_tensor.data()), max_blocks, true,
                ...
                cudaMemcpy(scratch.next_tokens.data(), next_tokens.data(), scratch.next_tokens.size() * sizeof(int),
                           cudaMemcpyDeviceToHost);
            ...
            if (gd_it == dec_graphs_.end() || use_split_decode)
            {
                nvtxRangePush("Engine_Decode_Fallback");
                run_decode_fallback(use_split_decode);
                nvtxRangePop();  // Engine_Decode_Fallback
        ...
        for (auto req : batch.requests)
        {
            ...
            if (hit_stop_token || req->generated_tokens.size() >= (size_t)req->max_tokens)
            {
                is_finished = true;
            }
            ...
            if (result_queue_ && (!valid_text.empty() || is_finished))
            {
                if (is_finished)
                {
                    result_queue_->push(req->id, valid_text, true, static_cast<int>(req->prompt_tokens.size()),
                                        static_cast<int>(req->generated_tokens.size()));
                }
                else
                {
                    result_queue_->push(req->id, valid_text, false);
                }
            }
